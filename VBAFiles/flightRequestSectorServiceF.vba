    ...
    On Error GoTo ErrorHandler
        ...
        If Not IsNull(selectedValue) Then
            ...
            Me.ACMTOW.Value = DLookup("ACMTOW", "ACRegT", "ACReg = '" & selectedValue & "'")
             Me.Operator.Value = DLookup("Operator", "ACRegT", "ACReg = '" & selectedValue & "'")
              Me.Customer.Value = DLookup("Customer", "ACRegT", "ACReg = '" & selectedValue & "'")
        Else
...
Private Sub addSectorBtn_Click()
    DoCmd.OpenForm "AddSectorF", WindowMode:=acWindowNormal
End Sub
...
Private Sub loadForm()
    ...
    If Me.Recordset.EOF Then
        ...
        Set db = CurrentDb
        strSQL = "SELECT TOP 1 AddedTime FROM NewFlightRequestsT ORDER BY AddedTime DESC"
        Set rs = db.OpenRecordset(strSQL)

        ' Check if there is a latest AddedTime value
        If Not rs.EOF Then
            rs.MoveFirst ' Move the recordset cursor to the first record

            latestAddedTime = rs.Fields("AddedTime").Value

            ' Retrieve the record with the latest AddedTime
            strSQL = "SELECT TOP 1 * FROM NewFlightRequestsT WHERE AddedTime = (SELECT MAX(AddedTime) FROM NewFlightRequestsT) ORDER BY AddedTime DESC"
            rs.Close
            Set rs = db.OpenRecordset(strSQL)
...
End Function
    ...
    // //// ' new function that generate random RefNo but makes sure that
        'new generated number is greater than the last one
        ...
        Private Function GenerateNewRefNo() As String
            ...
            Else
                ...
                ' Generate a random number greater than the last number
                Dim randomNumber As Long
                Randomize  ' Initialize the random number generator
                randomNumber = Int((999999 - lastNumber + 1) * Rnd + lastNumber)
